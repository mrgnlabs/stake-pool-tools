...
pub fn generate_spl_stake_pool_stats(
    rpc_client: &RpcClient,
    target_epoch_meta: &SplStakePoolMeta,
    next_epoch_meta: Option<SplStakePoolMeta>,
) -> EpochStakePoolStats {
    let (
        active_lamports,
    ...
    let next_epoch_lst_price = if let Some(next_epoch_meta) = next_epoch_meta {
        ...
        fetch_current_spl_stake_pool_price(
            &rpc_client,
...
fn fetch_current_spl_stake_pool_price(rpc_client: &RpcClient, stake_pool_address: Pubkey) -> f64 {
